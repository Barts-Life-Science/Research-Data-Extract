    ...
    INSERT INTO  BH_RESEARCH.DBO.RDE_FamilyHistory
          SELECT 
             ...
             --,CONVERT(VARCHAR(14),F.[RELATED_PERSON_ID])                     AS RELATED_PERSON_ID
   	         ,CONVERT(VARCHAR(14),REL.PERSON_RELTN_CD)                       AS RELATION_CD
			 ,CONVERT(VARCHAR(100),dbo.csvString(REF.CODE_DESC_TXT))         AS RelationDesc
			 ,CONVERT(VARCHAR(14),REL.PERSON_RELTN_TYPE_CD)                  AS RELATION_TYPE
...
IF @CritCare=1
  ...
  LEFT OUTER JOIN  [BH_DATAWAREHOUSE].[dbo]. PI_LKP_CDE_CODE_VALUE_REF TESTnm with (nolock) ON pev.EVENT_CD = TESTnm.CODE_VALUE_CD	
   WHERE (cce.EVENT_RESULT_UNITS_CD > 0 OR ISNUMERIC(cce.EVENT_RESULT_TXT) = 1 OR ISNUMERIC(LEFT(cce.EVENT_RESULT_TXT, 1)) = 1 
   OR UPPER(cce.EVENT_RESULT_TXT) = 'POSITIVE' OR UPPER(cce.EVENT_RESULT_TXT) = 'NEGATIVE'
	OR (
